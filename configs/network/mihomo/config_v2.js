...
// 自定义规则
const CUSTOM_RULES = [
  "DOMAIN-SUFFIX,linux.do,HK",
  "DOMAIN-SUFFIX,supercodex.space,HK",
  "DOMAIN-SUFFIX,anyrouter.top,HK"
];
...
const buildAiProxyList = (activeRegions = [], otherProxyNames = [], allNames =[]) => {
  const us = activeRegions.find((r) => r.name === "US")?.proxies || [];
  return us.length ? uniq(us) : allNames;
};
